...
class Camelyon_subtype(Dataset):
    ...
    def __init__(self, num_subImgs=8, data_root=None, istrain='train', fold_num=0):  
        ...
        if istrain=='train':
            df = pd.read_csv(f'data_scripts/csvs/camelyon16/reference_train_fold_{fold_num}.csv')
        elif istrain=='val':
            df = pd.read_csv(f'data_scripts/csvs/camelyon16/reference_val_fold_{fold_num}.csv')
        else:
