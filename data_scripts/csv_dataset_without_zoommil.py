...
class Camelyon_subtype(Dataset):
    ...
    def __init__(self, num_subImgs=8, data_root=None, istrain='train'):  
        ...
        if self.istrain:
            df = pd.read_csv('data_scripts/csvs/camelyon16/reference_train.csv')
        else:
...
class Camelyon_feature_subtype(Camelyon_subtype):
    ...
    def __init__(self, encoder_name='vit_base_patch16_224.owkin_pancancer', 
        ...
        self.encoder_name = encoder_name.split('/')[-1]
        self.feat_dir = os.path.join(self.data_root, 'features', f'skip_{skip_val}_{mag[0]}x', encoder_name)
        ...
        def get_feats(x, feat_dir):
            ...
            x = x.replace('.tif', '')
            feat_path = os.path.join(feat_dir, x, 'all_features.pt')
