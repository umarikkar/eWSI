...
class Mlp(nn.Module):
    ...
    def __init__(
        ...
        if lora_cfg[-2]:
            self.fc1 = lora.Linear(in_features, hidden_features, r=lora_rank, bias=bias[0])
        else:
        ...
        if lora_cfg[-1]:
            self.fc2 = lora.Linear(hidden_features, out_features, r=lora_rank, bias=bias[1])
        else:
...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        if True in lora_cfg[0:3]:
            self.qkv = lora.MergedLinear(dim, 3*dim, r=lora_rank, enable_lora=lora_cfg[0:3])
        else:
