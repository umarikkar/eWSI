...
from models.mil_utils_for_analysis.attention import *
from models.mil_utils_for_analysis.mlp import MLP
from models.mil_utils_for_analysis.tile_layers import TilesMLP
    ...
    def __init__(
        ...
        if use_tiles_embed:
            ...
            self.mlp = nn.Sequential(nn.Linear(in_features, hidden_dim), 
                                      nn.Sigmoid(), nn.Linear(hidden_dim, out_features))
...
class ABMIL(nn.Module):
    ...
    def __init__(
        self, in_features, out_features, hidden_dim = 128, use_tiles_embed=True
    ):
        super().__init__()
        ...
        self.attention_layer =  GatedAttention(d_model=attn_dim, temperature=1.0)
        self.mlp = nn.Linear(attn_dim, out_features)
    ...
    def forward(self, features):
        ...
        scaled_embs, _ = self.attention_layer(tiles_embs)
        logits = self.mlp(scaled_embs)
