...
class DoRALayer(nn.Module):
    def __init__(self, d_in, d_out, rank=4, weight=None, bias=None):
        ...
        self.lora_m = nn.Parameter(self.weight.norm(p=2, dim=0, keepdim=True))
    ...
    def forward(self, x):
        ...
        calc_weights = self.lora_m * norm_adapted
...
class SimpleModel(nn.Module):
    def __init__(self, input_dim, output_dim):
        ...
        self.layer2 = nn.Linear(output_dim, 1)
...
def replace_linear_with_dora_module(module, name, r=4):
    if isinstance(module, nn.Linear):
        d_in = module.in_features
        d_out = module.out_features
        setattr(model, name, DoRALayer(d_out=d_out, d_in=d_in, rank=r, weight=module.weight.data.clone(), bias=module.bias.data.clone()))

        return
    return


def replace_linear_with_dora(model, r=4):
