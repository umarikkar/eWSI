...
class SVDMergedLinear(nn.Linear, LoRALayer):
    def __init__(
        self, 
        in_features: int, 
        out_features: int, 
        ...
        **kwargs
        ...
        if r > 0 and any(enable_lora):
            ...
            self.lora_E = nn.Parameter(
                self.weight.new_zeros(r * sum(enable_lora), 1)
            ) 
            self.lora_B = nn.Parameter(
                self.weight.new_zeros((out_features // len(enable_lora) * sum(enable_lora), r))
            )
            self.ranknum = nn.Parameter(
                self.weight.new_zeros(sum(enable_lora)), requires_grad=False
            )
            self.ranknum.data.fill_(float(self.r))
            self.scaling = self.lora_alpha if self.lora_alpha>0 else float(self.r)   
            # Freezing the pre-trained weight matrix
    ...
    def train(self, mode: bool = True):
        ...
        if mode:
            ...
            if self.merge_weights and not self.merged:
                # Merge the weights and mark it
                if self.r > 0 and any(self.enable_lora):
                    self.weight.data += self.merge_ABE() * self.scaling
                self.merged = True   
...
class RankAllocator(object):
    ...
    def update_ipt(self, model):
        for n,p in model.named_parameters():
            if "lora_" in n: 
                if n not in self.ipt: # initialize the importance matrices
