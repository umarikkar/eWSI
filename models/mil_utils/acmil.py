...
from models.mil_utils.acmil_helpers import *
...
class Attention_Gated(nn.Module):
    ...
    def forward(self, x):
        ...
        A = self.attention_weights(A_V * A_U) # BxNxK
...
class ACMIL_GA_LinMax(nn.Module):
    def __init__(self, 
                 ...
                 D=128, droprate=0, n_token=1, n_masked_patch=0, mask_drop=0
                 ,use_tiles_embed=True, gate_only=False,):
        super(ACMIL_GA_LinMax, self).__init__()
        if use_tiles_embed:
            self.dimreduction = nn.Sequential(nn.Linear(D_feat, D_inner), 
                                                *[nn.Sequential(nn.Tanh(), nn.Linear(D_inner, D_inner)) for _ in range(2)])
        ...
        self.n_masked_patch = n_masked_patch
        self.n_token = conf_n_token
        self.Slide_classifier = Classifier_1fc(D_inner, n_class, droprate)
        self.mask_drop = mask_drop


    def forward(self, x): ## x: B x N x L
        
        x = x.squeeze(-2) # remove the magnification dimension
        # x = x[0]
        xx = self.dimreduction(x)
        AA = self.attention(xx)  ## B x K x N

        out1, out2, out3 = [],[],[]
        for A, x in zip(AA, xx):
            A = A.float()
            if self.n_masked_patch > 0 and self.training:
                # Get the indices of the top-k largest values
                k, n = A.shape
            ...
            out1.append(torch.stack(outputs, dim=0))
            out2.append(self.Slide_classifier(bag_feat))
            out3.append(A_out)

        return torch.stack(out1).squeeze(-1), torch.stack(out2).squeeze(-1), torch.stack(out3)
